...
description: >
  Use when 已有项目还没有完整 quick-sdd 工作区，需要初始化 `AGENTS.md`/`codespec/`，盘点现有功能、技术栈、模块边界与架构约束，并反向生成 baseline SDD 文档。
  Not for 发明新需求、实现代码、覆盖已有 codespec 产物、做 QA 质量裁决或 RA 最终验收。
  Output: baseline `proposal.md`、`stories.md`、`architecture.md`、`tasks.md`、`validation-report.md`、`acceptance.md` 与可续跑的 `state.json`。
---
...
- 如已存在：`AGENTS.md`、旧版 `AGENT.md`、`codespec/README.md`、`codespec/runtime/state.json`
- `../quick-sdd/templates/`
...
4. 更新 `AGENTS.md` 和 `codespec/README.md`，补齐项目目标、模块地图、关键约束和 baseline feature 索引
5. 按能力域或关键用户旅程拆 feature
6. 对每个 feature 生成 `proposal.md`、`stories.md` 和 `architecture.md`
7. 若当前没有明确开发请求，`tasks.md`、`validation-report.md` 和 `acceptance.md` 用 baseline 占位方式初始化

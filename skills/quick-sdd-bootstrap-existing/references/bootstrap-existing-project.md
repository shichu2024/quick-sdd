...
把当前系统的架构基线落成可续跑的设计文档：

- 记录模块边界、入口、关键依赖、数据/状态和外部集成
- 区分已经存在的架构事实与推断出来的设计意图
- 风险要落到具体模块、耦合点、数据迁移或接口契约，不写空泛概念
- 后续 DEV 编写 `tasks.md` 时应引用这里的架构决策或约束
...
如果当前只是接入 baseline，可以保留待验收状态，留给 RA 在 QA 之后续写：

- 写明当前为 baseline 初始化，尚未进行最终需求验收
- 不要伪造 `accepted`
- 如果已有 QA 报告或人工验收证据，只能作为 RA 后续验收输入
...
- architecture 反映真实模块边界和约束，不夹带未确认愿景
- acceptance 不伪造最终验收，只记录当前是否具备 RA 接受条件
